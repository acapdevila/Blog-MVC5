...
namespace LG.Web.Sitemap
{
    ...
    public class SitemapService : SitemapGenerator
    {
        ...
        protected virtual IReadOnlyCollection<SitemapNode> GetSitemapNodes()
        {
            ...
            foreach (var ruta in _buscadorRutas.BuscarRutasDeCategorias())
            {
                nodes.Add(
                    new SitemapNode(_urlHelper.AbsoluteRouteUrl(RouteConfig.NombreRutaCategoriaAmigable, new { urlCategoria = ruta.UrlSlug }))
                    {
                       // Frequency = SitemapFrequency.Weekly,
